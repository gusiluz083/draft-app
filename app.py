...
with left:
    ...
    for rival in data['rivals']:
        ...
        klass = 'rival-card rival-card-active' if active else 'rival-card'
        label = f"{rival['name']}\n{len(rival.get('matches', []))} partidos · {len(rival.get('players', []))} jugadoras"
        if st.button(label, key=f"select_{rival['id']}", use_container_width=True):
